...
interface UploadInput {
  ...
  enableSurfaceCompletion: boolean;
  surfaceCompletionModelPath: string;
...
export function SetupScreen({
  ...
  const [enableSurfaceCompletion, setEnableSurfaceCompletion] = useState(false);
  const [surfaceCompletionModelPath, setSurfaceCompletionModelPath] = useState("");
  ...
  const submit = (event: FormEvent) => {
    ...
    onUpload({
      ...
      enableSurfaceCompletion,
      surfaceCompletionModelPath: surfaceCompletionModelPath.trim(),
      ...
      <section className="launch-panel" aria-labelledby="new-run-title">
        ...
        <form onSubmit={submit}>
          ...
          <label className="check-field dense-option">
            <input
              type="checkbox"
              checked={enableSurfaceCompletion}
              onChange={(event) => setEnableSurfaceCompletion(event.target.checked)}
            />
            <span>Attempt AI hidden-surface prediction (visual only)</span>
          </label>
          {enableSurfaceCompletion && (
            <label className="field segmentation-model-field">
              <span>Local surface-completion weights <small>required</small></span>
              <input
                value={surfaceCompletionModelPath}
                onChange={(event) => setSurfaceCompletionModelPath(event.target.value)}
                placeholder="Local checkpoint path; nothing is downloaded automatically"
              />
            </label>
          )}
          ...
          <p className="policy-caveat">
            ...
            texturing. Primary-subject mode always requires a valid mask. AI-predicted surfaces
            are labelled separately and measurement is disabled for them.
          </p>
