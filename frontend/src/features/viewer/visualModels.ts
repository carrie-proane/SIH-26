import type { ViewerManifest, VisualMode } from "../../domain/contracts";
...
function modelForMode(mode: VisualMode, manifest: ViewerManifest) {
  ...
  if (mode === "PREDICTED") return manifest.visual_models?.ai_completed_mesh;
...
export function visualModeReason(mode: VisualMode, manifest: ViewerManifest): string {
  ...
  if (!model?.available || !model.url) {
    const label = mode === "TEXTURED"
      ? "Textured model"
      : mode === "PREDICTED"
        ? "AI predicted surface"
        : "Photoreal view";
    return model?.statement ?? `${label} was not declared by this run.`;
  }
  if (mode === "PHOTOREAL") {
