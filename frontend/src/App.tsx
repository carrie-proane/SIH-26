...
export default function App() {
  ...
  return <div className="app">
    ...
    {screen === "WORKSPACE" && bundle && <Workspace bundle={bundle} project={project} run={run} readiness={readiness} actionBusy={actionBusy} globalError={error} onReset={reset} onCancel={run ? () => void runAction("cancel", () => cancelRun(run.run_id)) : undefined} onResume={run ? () => void runAction("resume", () => resumeRun(run.run_id)) : undefined} onRerun={(config: RunConfiguration) => run && void runAction("rerun", () => rerunRun(run.run_id, config))} onCompletionChanged={() => run && void openExistingRun(run.run_id)} />}
  </div>;
}
