...
from ..reconstruction.confidence import confidence_contract, validate_point_confidence_for_ply
...
def build_viewer_manifest(record: RunRecord, run_dir: Path) -> dict[str, Any]:
    ...
    completed_mesh_path = (
        "completion/completed_mesh.ply"
        if "completion/completed_mesh.ply" in artifacts
        else None
    )
    ...
    return {
        ...
        "visual_models": {
            ...
            "ai_completed_mesh": {
                "available": completed_mesh_path is not None,
                "url": artifacts[completed_mesh_path].url if completed_mesh_path else None,
                "format": "PLY" if completed_mesh_path else None,
                "coordinate_frame": "LOCAL_ENU_METRES" if completed_mesh_path else None,
                "measurement_eligible": False,
                "statement": (
                    "AI-predicted hidden surfaces - visual hypothesis only, not measurable"
                    if completed_mesh_path
                    else "AI surface completion was not produced for this run."
                ),
            },
            "dense_report_url": (
            ...
            "completion_report_url": (
                artifacts["completion_report.json"].url
                if "completion_report.json" in artifacts
                else None
            ),
        },
        "camera_path": {
